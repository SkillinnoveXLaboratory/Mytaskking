...
Future<void> showDesktopThemeColorEditor(
  BuildContext context, {
  required DesktopThemeId themeId,
}) {
  return showDialog<void>(
    context: context,
    barrierDismissible: true,
    builder: (ctx) => _DesktopThemeColorEditorDialog(themeId: themeId),
  );
}

class _DesktopThemeColorEditorDialog extends StatefulWidget {
  const _DesktopThemeColorEditorDialog({required this.themeId});

  final DesktopThemeId themeId;

  @override
  State<_DesktopThemeColorEditorDialog> createState() =>
      _DesktopThemeColorEditorDialogState();
}

class _DesktopThemeColorEditorDialogState
    extends State<_DesktopThemeColorEditorDialog> {
  ...
  void initState() {
    super.initState();
    final base = DesktopThemePalettes.basePaletteFor(widget.themeId);
    final saved = DesktopLocalSettings.overridesFor(widget.themeId);
  ...
  Future<void> _save() async {
    ...
    await DesktopLocalSettings.setThemeColorOverrides(widget.themeId, overrides);
    if (!mounted) return;
  ...
  Future<void> _reset() async {
    ...
    await DesktopLocalSettings.resetThemeColorOverrides(widget.themeId);
  }
            ...
            Expanded(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final colorList = ListView.builder(
                    padding: const EdgeInsets.only(left: 20, right: 8),
                    itemCount: BestiePaletteEditing.editableFields.length,
                    itemBuilder: (context, index) {
                      final field = BestiePaletteEditing.editableFields[index];
                      final color = Color(
                        _draft[field.key] ??
                      ...
                      return ListTile(
                        ...
                        onTap: () =>
                            setState(() => _selectedKey = field.key),
                      );
                    },
                  );
                  final maker = SingleChildScrollView(
                  ...
                  if (stacked) {
                    return Column(
                      children: [
                        Expanded(flex: 3, child: colorList),
                        const Divider(height: 1),
                        Expanded(flex: 4, child: maker),
                      ],
                    );
                  }
                  return Row(
